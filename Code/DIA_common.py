...
def make_reference(files,reg,params,reference_image='ref.fits'):
    ...
    if params.ref_include_file:
        ...
        for f in files:
            ...
            if (f.fw < ref_seeing) and (f.fw > params.reference_min_seeing) and (f.signal > sigcut) and not(f.name in reference_exclude):
                ref_sky = f.sky
                ref_seeing = f.fw
                best_seeing_ref = f
        ...
        while len(ref_list) < params.min_ref_images:
            ...
            for f in files:
                if (f.fw < params.reference_seeing_factor*ref_seeing) and (f.sky < params.reference_sky_factor*ref_sky) and (f.fw > params.reference_min_seeing) and (f.signal > sigcut) and not(f.name in reference_exclude):
                    ref_list.append(f)
                    print f.name, f.fw, f.sky, f.signal
